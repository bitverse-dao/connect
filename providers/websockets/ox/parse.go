...
func (h *WebSocketHandler) parseSubscribeResponseMessage(resp SubscribeResponseMessage) ([]handlers.WebsocketEncodedMessage, error) {
	...
	if t := EventType(resp.Event); t == EventSubscribe {
		h.logger.Debug("successfully subscribed to channel", zap.String("instrument", resp.Channel))
		return nil, nil
	}

	//
	// The message is an exact copy of the request message, so we can just unmarshal it and re-subscribe.
	h.logger.Debug("received error message", zap.String("message", resp.Channel), zap.Bool("Success", resp.Success))
	jsonString := strings.Split(resp.Channel, ExpectedErrorPrefix)
	if len(jsonString) != ExpectedErrorElements {
		return nil, fmt.Errorf("unable to parse subscription message from message: %s", resp.Channel)
	}
...
func (h *WebSocketHandler) parseTickerResponseMessage(
	...
	for _, instrument := range resp.Data {
		ticker, ok := h.cache.FromOffChainTicker(instrument.MarketCode)
		if !ok {
			h.logger.Debug("ticker not found for instrument ID", zap.String("instrument_id", instrument.MarketCode))
			continue
		}

		// Convert the price to a big.Float.
		price, err := math.Float64StringToBigFloat(instrument.MarketPrice)
		if err != nil {
